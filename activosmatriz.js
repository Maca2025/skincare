...
// calibración UVA.
//
// ALTA 2026-08-17 · 5 productos, 87 → 92 entradas en PRODUCT_DOSE y
// PRODUCT_ZONAS, e IRRITANTES de 12 → 14. Dos de ellos son medicamentos de
// receta y el detalle de su calibración está en claude/dos-retinoides-2026-08-17.md.
// ══════════════════════════════════════════════════════════════════════════
const FUNCIONES = {
...
const PRODUCT_DOSE = {
  ...
  // RECETA. Adapaleno 0.3%, la concentración más alta que existe. `textura` a
  // la par de la tretinoína 0.025% y `aclarado` por encima de ella, apoyado en
  // Bagatin 2018 (Eur J Dermatol): adapaleno 0.3% gel ≈ tretinoína 0.05% crema
  // en fotoenvejecimiento (efélides/melanosis 64.9% vs 71.9%, p=0.546), o sea
  // ENTRE las dos tretinoínas de esta matriz. Su evidencia propia en léntigos
  // es Kang 2003 (JAAD, n=90): 59% vs 36% del vehículo — real, pero exigió DOS
  // aplicaciones diarias durante nueve meses, y por eso no llega a 90.
  ...
  // Vitamina C 15% en tres formas. Por debajo del Suero Vitamina C 20% en los
  // tres ejes: es menos concentrado y DOS de sus tres formas son derivados
  // (3-O-etil ascórbico y tetraisopalmitato), que según Pinnell 2001 no elevan
  // el nivel cutáneo de ácido L-ascórbico. El L-ascórbico puro va séptimo en el
  // INCI, después del Alcohol Denat. — que va TERCERO.
  ...
  // RECETA. 4× la concentración de la de arriba, y la más alta que Retin-A
  // vende en México. Es el ÚNICO producto de esta matriz con evidencia de nivel
  // 1 sobre léntigos solares — que es el diagnóstico de la usuaria — y los dos
  // ensayos usaron EXACTAMENTE 0.1%, no 0.025%: Rafal 1992 (NEJM, n=58, 10
  // meses) 83% de pacientes aclararon vs 29% del vehículo, sin recaída a los 6
  // meses de suspenderla; Griffiths 1994 (JAAD, n=45) 90% vs 33%, con −41% de
  // pigmentación epidérmica en histología contra +37% del vehículo. De ahí que
  // `aclarado` suba 70 → 90 y empate al Finacea. `textura` va a 100 porque el
  // 0.025% ya ocupaba el techo de 95 y esto es cuatro veces más fuerte.
  // ⚠️ Fotolábil: bajo UVA pierde >70% del activo en 8 horas. Solo de noche.
  ...
  // Mismo `barrera` que el Peptide + HA Serum: los dos hidratan de verdad, y
  // aquí la urea va en posición 8 del INCI, ANTES del conservador. Pero
  // `firmeza: 20` contra los 55 de aquél, porque el de The Ordinary es un sérum
  // de péptidos y aquí el Palmitoyl Tripeptide-38 va ÚLTIMO de 32 ingredientes
  // y la ubiquinona en la 24. La mejoría inmediata que anuncia la marca ("71%
  // de usuarias en 30 minutos") es un formador de película — Polyacrylamide con
  // C13-14 Isoparaffin y siliconas — no biología: tensa mientras está puesto.
  // ⚠️ CINCO alérgenos de fragancia declarados además del Parfum (Limonene,
  // Hexyl Cinnamal, Coumarin, Citral, Linalool). Nivel Caudalie Beauty Elixir.
...
const PRODUCT_ZONAS = {
  ...
  // Solo cara al arrancar, y es deliberado: es GEL con propilenglicol y reseca
  // más que una crema. Su categoría '💊 Activos' ya lo hace apto en cuello, así
  // que `zonasAptasDe` deja el chip disponible — simplemente no lo
  // preselecciona. Se agrega 'cuello' a las 4 semanas si no irrita. Mismo
  // criterio que el Zero Pore Pad.
  ...
  // Cara y cuello igual que el 0.025%: claude/cadencia-especificacion.md §2
  // registra que la usuaria tolera muy bien la tretinoína en cuello (piso 2 /
  // techo 6). Rafal 1992 también trató extremidades superiores — si se quiere
  // en manos, se agrega 'manos' y el eje `manos` gana munición.
...
const IRRITANTES = new Set([
  ...
  // Los dos retinoides de receta que entraron el 2026-08-17. Cada uno es la
  // concentración MÁS ALTA que existe de su molécula, y las fichas de ambos
  // desaconsejan usarlos con otro retinoide. Perseguida hasta su origen (ficha
  // europea de Differine §4.5), esa advertencia NO es incompatibilidad química
  // —el adapaleno es "estable a la luz y al oxígeno y químicamente no
  // reactivo"— sino redundancia de mecanismo más suma de irritación. No existe
  // ningún ensayo de tretinoína + adapaleno usados juntos ni alternados.
  //
  // ⚠️ Con estas dos, los IRRITANTES aptos para CARA pasan de 9 a 11, y el eje
  // `textura` tiene diasTecho: 6 con la regla de uno por noche. El calendario
  // no cierra: repartir significa bajar la tretinoína 0.1% —la única con
  // evidencia de nivel 1 en léntigos— a 2 o 3 noches, cuando su ficha pide
  // diario 10-12 semanas antes del mantenimiento. Es una decisión clínica
  // pendiente, no un problema de código. Ver claude/dos-retinoides-2026-08-17.md.
